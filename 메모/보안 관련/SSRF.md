...
```
POST /preview{  "url": "https://example.com"}
```
...
```java
URI uri = URI.create(url);InetAddress address = InetAddress.getByName(uri.getHost());if (address.isLoopbackAddress()    || address.isSiteLocalAddress()) {    throw new SecurityException("SSRF detected");}
```
