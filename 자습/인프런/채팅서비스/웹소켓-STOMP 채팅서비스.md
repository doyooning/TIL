...
---
### 실시간 채팅 등 구현시 서버 아키텍쳐

SSE, 실시간 채팅 등 서버 메모리 의존적인 기능 구현시 로컬에선 잘 되는데 배포할 때 안 되는 경우가 많음
특히 서버를 여러 대 운영하면 발생
-> 한 명의 클라이언트가 하나의 서버에서 메세지를 보냈을 때 다른 서버의 사용자들도 볼 수 있어야 함

**Redis의 publish, subscribe 기능을 통해 해결**
(kafka도 동일한 기능이 존재)
모든 서버가 Redis를 listen하는 상태(subscribe)
여기서 클라이언트가 메시지 전송(publish)하면 
-> 서버가 Redis로 메시지 전송
-> Redis는 listen 상태인 모든 서버에 메시지를 뿌려줌
-> 클라이언트는 자기 자신이 보낸 메시지도 볼 수 있고(문제x), 다른 사람들도 볼 수 있음
